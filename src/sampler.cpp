...
void Sampler::sample() {
  ...
  for (arma::uword i = 0; i < dat.iter; i++) {
    for (arma::uword j = 0; j < dat.thin; j++) {
      ...
      pars.update_delta_phi(dat, transf);
      // pars.update_xi_phi(dat, transf);
      pars.update_delta_lambda(dat, transf);
      // pars.update_xi_lambda(dat, transf);
...
void Sampler::write_samples() {
  ...
  pars.alpha_container(current_iter) = pars.alpha;
  pars.delta_lambda_container.col(current_iter) = pars.delta_lambda;
  pars.xi_lambda_container.slice(current_iter) = pars.xi_lambda;
  pars.delta_phi_container.slice(current_iter) = pars.delta_phi;
  pars.xi_phi_container(current_iter) = pars.xi_phi;
  current_iter++;
}
...
Rcpp::List Sampler::get_samples() {
  return Rcpp::List::create(Rcpp::Named("lambda", pars.lambda_container),
                            ...
                            Rcpp::Named("delta_lambda", pars.delta_lambda_container),
                            Rcpp::Named("xi_lambda", pars.xi_lambda_container),
                            Rcpp::Named("delta_phi", pars.delta_phi_container),
                            Rcpp::Named("xi_phi", pars.xi_phi_container),
