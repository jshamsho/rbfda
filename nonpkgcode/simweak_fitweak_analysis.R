...
which(!sapply(1:300, function(i) any(dir() == paste0("n50_simpartial_fitweak", i, ".RData"))))
nreg <- 6
ldim <- 4
nt <- 60
nsub <- 50
...
for (num in 1:nsim) {
  ...
  for (r in 1:nreg) {
    ...
    prop_coverage2 <- mean((simstats$eigenvec_summary[,r,2] <= -true_eigenvec[,r]) &
                             (simstats$eigenvec_summary[,r,3] >= -true_eigenvec[,r]))
    eigenvec_accuracy[r, num] <- min(prop_accuracy1, prop_accuracy2)
    eigenvec_coverage[r, num] <- max(prop_coverage1, prop_coverage2)
  }
