library(refund)
initialize_mcmc <- function(Y, tt, nt, B, X, ldim = NULL) {
  nsub <- nrow(Y) / nt
  nreg <- ncol(Y)
  L <- list()
  omega <- numeric(nreg)
  ...
  if (is.null(ldim)) {
    fpca1 <- refund::fpca.face(t(Yt), center = TRUE, knots = floor(length(tt) / 5))
  } else {
  ...
  #   fpca1 <- FPCA(Ly, Lt, optns = list(dataType = 'Dense',  methodnpc = ldim))
  # } else {
